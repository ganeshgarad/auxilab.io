  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__auxilabThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__auxilabArtifact__', '__auxilabThought__', '__auxilabQuickAction', '__auxilabSelectedElement__'],

      // ['className', '__auxilabThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__auxilabArtifact__', '__auxilabThought__', '__auxilabQuickAction'],
    ],
  },
  strip: [],
};
