...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          auxilab_user_profile: this._safeGetItem('auxilab_user_profile'),
          auxilab_settings: this._safeGetItem('auxilab_settings'),
          auxilab_profile: this._safeGetItem('auxilab_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('auxilab_viewed_features'),
          developer_mode: this._safeGetItem('auxilab_developer_mode'),
        ...
        ui: {
          // Tab configuration
          auxilab_tab_configuration: this._safeGetItem('auxilab_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('auxilab_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('auxilab_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          auxilab_read_logs: this._safeGetItem('auxilab_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('auxilab_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('auxilab_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.auxilab_tab_configuration) {
        try {
          this._safeSetItem('auxilab_tab_configuration', data.ui.auxilab_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'auxilab_acknowledged_debug_issues',
        'auxilab_acknowledged_connection_issue',
        'error_logs',
        'auxilab_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('auxilab_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
