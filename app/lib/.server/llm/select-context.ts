...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyauxilabActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyauxilabActions(content);

      content = content.replace(/<div class=\\"__auxilabThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`auxilab failed to select files`);
  }
