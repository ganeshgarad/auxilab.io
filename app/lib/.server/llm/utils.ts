...
export function simplifyauxilabActions(input: string): string {
  // Using regex to match auxilabAction tags that have type="file"
  const regex = /(<auxilabAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/auxilabAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<auxilabAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</auxilabAction>`;
    });

  return `<auxilabArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</auxilabArtifact>`;
}
